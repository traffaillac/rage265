...
static const Rage265_picture *parse_slice_segment_layer(Rage265_ctx *r, unsigned int lim) {
    ...
    unsigned int used_for_reference = r->needed_for_output;
    ...
    if (unavailable_poc > INT32_MIN) {
        unsigned int i, avail = 0, num_ctbs = s.p.PicWidthInCtbsY * s.p.PicHeightInCtbsY;
        for (i = 0; i < s.p.max_dec_pic_buffering && r->DPB[i].blank_ctbs < num_ctbs; i++) {
            if (!(used_for_reference & (1 << i)))
                avail = i;
        }
        if (i == s.p.max_dec_pic_buffering) {
            i = avail;
            r->DPB[i].blank_ctbs = num_ctbs;
    ...
    for (i = 0; i < s.p.max_dec_pic_buffering && r->DPB[i].PicOrderCntVal != s.currPic.PicOrderCntVal; i++) {
        if (!(used_for_reference & (1 << i)))
            avail = i;
    }
    if (i == s.p.max_dec_pic_buffering) {
        i = avail;
        r->DPB[i].blank_ctbs = 0;
        r->DPB[i].PicOrderCntVal = s.currPic.PicOrderCntVal;
    }
    s.currPic = r->DPB[i];
    r->needed_for_output |= pic_output_flag << i;
    ...
    if (num_reordered_pics <= s.p.max_num_reorder_pics && r->nal_unit_type < 20 &&
        ...
        r->needed_for_output ^= 1 << (output - r->DPB);
    return output;
}
...
static const Rage265_picture *parse_end_of_seq(Rage265_ctx *r, unsigned int lim) {
    ...
    if (lim == 0) {
        for (unsigned int i = 0; i < r->SPS.max_dec_pic_buffering; i++) {
            if ((r->needed_for_output & (1 << i)) && (output == NULL ||
...
const Rage265_picture *Rage265_parse_NAL(Rage265_ctx *r, const uint8_t *buf, size_t len) {
    ...
    if (len < 2)
        return NULL;
    len = umin(len, 100000000); // 800000 * 1000 / 8
